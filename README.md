# Anna & Gordan's energy registrations
...
There are many different data flows, and they are identified using the file extension (.TRF, .CNF) etc.
...
### Notes
- **For this exercise we only care about the "Registration" data flow (.CNF)**
- Each row in the data flow corresponds to a single registration and is identified by the `registration_id`
- Files names are a 6 digit sequential numbers `000001.CNF`, `000002.CNF` - you must not use the same number twice, and you must not upload a smaller number than the previous one
- There is a cost associated to every file that we upload
